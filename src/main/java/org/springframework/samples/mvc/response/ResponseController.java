...
import org.springframework.samples.mvc.convert.JavaBean;
...
public class ResponseController {
	...
	@GetMapping("/entity/bla")
	public ResponseEntity<JavaBean> responseEntityBla() {
		HttpHeaders headers = new HttpHeaders();
		headers.setContentType(MediaType.APPLICATION_JSON);
		if (headers.containsKey("das")) {
			return ResponseEntity.notFound().build();
		} else if (headers.containsKey("sadas")) {
			return ResponseEntity.status(202).body(new JavaBean());
		}
		ResponseEntity<JavaBean> b = ResponseEntity.ok(new JavaBean());
		return b;
	}
